...
- complete Mind Contract CI green on the release commit;
...
- tag `v0.9.0` pointing at the exact verified release commit;
...
A formal release is a separate publication action performed only after the verified source commit is known. The release must point to that exact commit and must not rerun or duplicate correctness checks unless the release input or environment has materially changed.
